...
def prepare_weights_for_lp(state_dict, model_key):
    print(model_key)
    ...
    if model_key == 'odin1':
        ...
        state_dict = {
            ...
            .replace('norm.', 'ln1.'): v for k, v in state_dict.items() if k.startswith(rel_key)
        }
        ...
        state_dict = {
            ...
            .replace('.attn.qkv.', '.attn.attn.in_proj_')
            .replace('.attn.projection.', '.attn.attn.out_proj.')
            .replace('norm.', 'ln1.'): v for k, v in state_dict.items()
        }
        ...
        state_dict = {
            ...
            .replace('.attn.qkv.', '.attn.attn.in_proj_')
            .replace('.attn.projection.', '.attn.attn.out_proj.')
            .replace('norm.', 'ln1.'): v for k, v in state_dict['teacher'].items()
        }
    else:
        state_dict = {
            k.replace('backbone.', '')
...
class VisionTransformer(BaseModule):
    ...
    def init_weights(self):
        if (isinstance(self.init_cfg, dict)
            ...
            state_dict = prepare_weights_for_lp(state_dict, weight_model)

            model_params = [k for k, _ in self.named_parameters()]
            param_diff = set(model_params) - set(state_dict.keys())
            if param_diff:
                print(f'ERROR - Params difference: {param_diff}')
                print(f'Expected params: {model_params}')
                print(f'Passed params: {state_dict.keys()}')
                print('Stopping run - Please fix naming in the relevant state dict!')
                raise ValueError(f"Params from state dict {self.init_cfg['checkpoint']} don't match!")
